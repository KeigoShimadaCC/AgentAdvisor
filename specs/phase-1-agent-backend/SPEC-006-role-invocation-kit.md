...
- `orchestrator/roles_config.py`: role registry loaded from `cursor/roles/roles.yaml`: per role → role md path, default model, escalation model, read_only flag, projection include-list, output artifact type. Model-family helper (`family(model_id)`) for the Director≠Challenger check.
- `orchestrator/workspace.py`: builds `cases/<id>/agents/<role>--<task-id>/` containing the role md written as the workspace `AGENTS.md`, `inputs/*.yaml` (projected artifacts), `task.yaml` (assignment + required output filename + schema name), empty `outputs/`.
...
- [ ] `cursor/roles/roles.yaml` (skeleton entries for all eight roles with the model mapping from the 2026-07-30 research report)
...
- [ ] Workspace contains exactly AGENTS.md, task.yaml, inputs/, outputs/ and nothing else; role md content equals workspace AGENTS.md.
