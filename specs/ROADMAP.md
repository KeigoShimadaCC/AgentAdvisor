...
of wall clock, ~7M tokens). `make check` is green: lint, mypy, **934 unit tests**, plus 18 live
...
| SPEC-057 | Deck deliverable generated at case completion | verified |
...
- (2026-08-04) **Consulting deck output for a completed case.** A repo-local skill now exists at
  ...
  **Promoted to Phase 8 on 2026-08-06** as SPEC-057 (draft), at user direction after a demo deck was
  generated by hand from the fixture case. The spec settles all three questions: the deck lives in
  `cases/<id>/outputs/deck/`; it is not gated on review because it is generated after `done` and can
  only reflect approved content; and matplotlib becomes an optional `deck` dependency group with a
  table-exhibit fallback, so a default install still builds a deck (the one item awaiting sign-off).
  Generation is deterministic (no agent, no budget) and non-fatal, mirroring `_record_into_memory`.
  **Implemented and verified on 2026-08-08** (`orchestrator/deck.py`, `deck_template.py`,
  `deck_charts.py`, the pipeline hook, `advisor deck`, three lexicon entries and the optional
  `deck` group): `make check` green, `tests/test_deck.py` (10) plus a deck assertion in the stub
  e2e pass, and the fixture deck renders 11 slides with zero geometry errors.
...
- ~~(2026-08-03) **Synthesis-stage projection truncation (found in SPEC-020).**~~ **Fixed 2026-08-06.** The SPEC-020 real case's synthesizer stated in its own output that the preliminary recommendation, objection resolutions, and pre-mortem leading indicators "were truncated out of the inputs available to this synthesis," and the reviewer then blocked twice on `verification.undisclosed_open_objection` and `verification.uncited_claim`. Root cause: `project()` filled a flat 20,000-char budget greedily in include-list order, so `evidence_records` at position four could consume it before the must-cite inputs behind it were reached — and SPEC-040 then inserted `ach_matrix` at position one, ahead of everything. Measured on the fixture case the include set is 11.7k; scaled to SPEC-020's 19 evidence / 7 objections it runs ~30k against the 20k budget, so roughly a third of the inputs were being dropped from the tail. Fixed by `projection_required` in the role config: named keys are resolved and given budget ahead of everything else, whatever their position, and a required key that cannot fit raises instead of vanishing. The synthesizer declares six (`decision_spec`, `artifact_index`, `objections`, `preliminary_recommendation`, `premortem_report`, `review_report`), which measure ~5-10k and leave room for evidence. The budget itself was left at 20k: prioritisation was enough, and raising it changes cost on every role. Guarded by `test_role_synthesis.py`, which reproduces the crowding-out and then asserts the shipped config prevents it.
